				...
				<view class="ts-row ts-flex-item">
					...
					<view class="ts-column ts-flex-item ts-padding-left ts-padding-right" style="height: 150upx; justify-content: space-between;">
						<view class="ts-column">
							...
							<view class="ts-row ts-h5">{{item.company}}</view>
						</view>
						<view class="ts-row" style="justify-content:space-between;">
							<view class="ts-row">
								<view class="ts-padding-right ts-h5">{{item.city}}</view>
								<view class="ts-padding-right ts-h5">{{item.diplomas}}</view>
								<view class="ts-h5">{{item.working_years}}</view>
							</view>
							<view class="ts-h5">
								{{item.date}}
							</view>
	...
	export default {
		...
		methods: {
			...
			loadMoreSearchResult () {
				...
				uni.request({
					url: this.$API_BASE + `/app/syb/searchJob`,
					method: 'GET',
					dataType: 'json',
					data: {
						page: this.page,
